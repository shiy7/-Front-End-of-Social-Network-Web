...
import (
	...
	// libraries to protect post and search with JWT
	jwtmiddleware "github.com/auth0/go-jwt-middleware"
	jwt "github.com/dgrijalva/jwt-go"
)

// add POST_INDEX, DISTANCE and ES_URL as a default search range
const (
	POST_INDEX = "post"
	DISTANCE   = "200km"

	ES_URL      = "http://10.128.0.9:9200"
	BUCKET_NAME = "lai_project2_bucket"
...
// define the struct for post and location
// Encode JSON object
type Post struct {
...
func main() {
	...
	//  to protect post and search with JWT, and also add signup and login endpoints
	...
	// not need add token, these two are to generate token
...
// user to send a HTTP request with a body, then the method will automatically construct a Post object named p and update its value
// to parse multiform requests and save images to GCS
func handlerPost(w http.ResponseWriter, r *http.Request) {
	// Parse from body of request to get a json object.
	fmt.Println("Received one post request")

	w.Header().Set("Content-Type", "application/json")
	// To exit early for OPTIONS method
	...
	// Update handlerPost to use username inside of token
	user := r.Context().Value("user") // user is key name, cannot be changed. Token is the value of user.
...
// To return post data read from Elasticsearch
...
// To handler GET request sent to /cluster
...
// To read posts from Elasticsearch
...
// to save a user post to Elasticsearch
// interface{} to support saving different kinds of data, save both user records and post records
